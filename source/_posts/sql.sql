...
-- 数据接入150张表。核心表50张
-- cdm层开发。dwd 50张，dws 8张,一致性维度表 dim10张
-- 开发三个项目的定制ads指标20个
-- 项目交付周期缩减30%以上
-- 沉淀出几个核心的数据资产。企业画像模块，政策分析模块，知产查询模块，为公司打造课程服务产品提高了一个基础的数据底座服务
-- 引入数据分析平台Starrocks后，下游数据产品，数据分析，取数分析更加方便快捷。从之前3工时左右的耗时，到自主查询十分钟解决
-- 或者联系数仓团队1工时左右解决